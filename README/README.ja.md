# 🎶 Whisper Mate – ローカルファーストの字幕・音声ワークスペース
[Deutsch](README.de.md) · [English](../README.md) · [Spanish](README.es.md) · [Finnish](README.fi.md) · [French](README.fr.md) · [Italian](README.it.md) · [Indonesian](README.id.md) · [언어](README.ko.md) · [日本語](README.ja.md) · [简体中文](README.zh_cn.md) · [繁体中文](README.zh_tw.md) · [Norwegian](README.nb.md) · [Dutch](README.nl.md) · [Polish](README.pl.md) · [Portuguese](README.pt_PT.md) · [Swedish](README.sv.md) · [ภาษาไทย](README.th.md) · [Turkish](README.tr.md) · [Ukrainian](README.uk.md) · [Vietnamese](README.vi.md)

| ![Whisper Mate icon](../images/appicon-128x128.png) | Whisper Mate は音声や動画を、洗練された字幕・翻訳・エクスポートへ変換します。すべてを Mac 上でローカル処理し、プロジェクトの機密性を守ります。macOS App Store でダウンロード: [Whisper Mate](https://apps.apple.com/us/app/id6450404233) |
| --- | --- |

## 🔑 主な特長
- **プライバシーを最優先**: 文字起こし・翻訳・エクスポートを完全ローカルで実行。機密データが Mac の外へ出ることはありません。
- **最適なエンジンを選択**: FluidAudio、whisper.cpp、WhisperKit、Apple のオンデバイス AI（macOS 26+）、Kokoro English Speech Synthesis を用途に合わせて切り替え。
- **ライブ制作ワークフロー**: 録音、字幕、翻訳、フローティングプレビュー、オートメーションチェーンをリアルタイムで監視。
- **仕上がりそのままのプレビュー**: 内蔵プレーヤーで字幕の書体・サイズ・行間をレスポンシブに確認。
- **拡張可能なツール群**: テンプレート、JavaScript プロセッサー、辞書、正規表現変換を組み合わせて高度な字幕整形を自動化。
- **字幕専用デスク**: 編集、レビュー、バッチ入出力、Final Cut Pro / QuickLook / GIF / MKV パイプラインまでワンストップで管理。

## 🔥 主要機能
- **バッチ & ライブ文字起こし**: 大量メディア、ライブマイク入力、アプリ音声ルーティング、同時翻訳まで対応。
- **字幕・翻訳エディタ**: 結合、分割、タイミング調整、Quick-Cut、置換、キーボード中心の操作を提供。
- **多言語翻訳**: DeepL、Apple Translate、GPT、Gemini、DeepSeek、OpenRouter、Ollama 互換のローカル LLM を利用可能。
- **自動化プレイブック**: ジョブ完了後にスクリプト、エクスポート、要約、通知を自動実行。
- **入出力エコシステム**: SRT、VTT、ASS、FCPXML、GIF、PDF、DOCX、MKV、オーディオクリップなどをシームレスに扱う。
- **ビジュアルツール**: フローティングウィンドウ、柔軟なフォント・行間設定、テキストのみ・分割ビューを即座に切替。
- **LLM 連携**: Apple オンデバイスモデル、Ollama、リモート LLM と接続し、字幕の磨き上げ・要約・スクリプト修正に活用。
- **プロジェクト管理 & スナップショット**: プロジェクトのグループ化、キュー優先度設定、リバーシブルなスナップショット保存、安全なアーカイブ、リモートフォルダ監視。
- **動画ポストプロダクション**: Quick Cut 波形編集、チャプター、シーンキーフレーム、GIF/MP4 エクスポート、ハードサブ埋め込み。

## 🚀 ユースケース
- 映画、ポッドキャスト、インタビューの字幕制作・翻訳・ナレーション。
- 会議、授業、ライブ配信向けリアルタイムメモと要約。
- 多言語バッチを自動化して処理するメディアチーム。
- スナップショットで音声コーパスをアーカイブ・注釈付けする学術/研究チーム。
- 長尺動画をショート、GIF、ナレーション付きクリップへ再構成するマーケ/ソーシャルチーム。
- 高品質な監査ログが必要なサポート、営業、法務向け録音のコンプライアンス対応。

## 📋 上級リソースガイド
- **FAQ & Tips**: [近日公開]
- **カスタムエクスポートテンプレート**: [近日公開]
- **LLM 連携（要約・QA・修正）**: [近日公開]
- **字幕付き動画の書き出し**: [近日公開]
- **ディレクトリ監視 & 自動文字起こし**: [近日公開]
- **リアルタイム文字起こし**: [近日公開]
- **セグメント再文字起こし**: [近日公開]
- **プリセット管理**: [近日公開]
- **辞書ワークフロー**: [近日公開]
- **スナップショット活用ベストプラクティス**: [近日公開]
- **マルチプロジェクト管理 & 検索**: [近日公開]
- **完全自動ワークフロー事例**: [近日公開]

## 💬 サポート & フィードバック
- アプリ内の **About** メニューからフィードバックを送るか、GitHub で Issue を作成して進捗を一緒に追跡してください。

## 🏞️ アプリ画面
![Projects](../images/v9-whispermate_main.webp)
![Subtitle Editor](../images/v9-sub-editor.webp)
![Live Transcription](../images/v9-live-video-transcription.webp)
![Embed Video Previewer](../images/v9-floating.webp)
![Re-transcription](../images/v9-retranscription.webp)
![Custom Export Template](../images/v9-custom-template.webp)

## 🔋 素晴らしいオープンソースプロジェクトに感謝

- [![GitHub stars](https://img.shields.io/github/stars/FluidInference/FluidAudio.svg?style=social&label=Star%20FluidAudio)](https://github.com/FluidInference/FluidAudio)
- [![GitHub stars](https://img.shields.io/github/stars/argmaxinc/WhisperKit.svg?style=social&label=Star%20WhisperKit)](https://github.com/argmaxinc/WhisperKit)
- [![GitHub stars](https://img.shields.io/github/stars/ggerganov/whisper.cpp.svg?style=social&label=Star%20whisper.cpp)](https://github.com/ggerganov/whisper.cpp)

<p align="center">
  <a href="https://fluidinference.com"><img src="https://assets.inference.plus/fi-badge.png" alt="Powered by Fluid Inference" height="80"></a>
  <a href="https://github.com/ggerganov/whisper.cpp" style="margin-left: 16px;"><img src="https://user-images.githubusercontent.com/1991296/235238348-05d0f6a4-da44-4900-a1de-d0707e75b763.jpeg" alt="Powered by whisper.cpp" height="80"></a>
  <a href="https://github.com/argmaxinc/WhisperKit" style="margin-left: 16px;"><img src="../images/vender/whisperkit.png" alt="Powered by WhisperKit" height="80"></a>
</p>
