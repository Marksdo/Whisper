# 🎶 Whisper Mate – Lokale werkruimte voor ondertitels en spraak
[Deutsch](README.de.md) · [English](../README.md) · [Spanish](README.es.md) · [Finnish](README.fi.md) · [French](README.fr.md) · [Italian](README.it.md) · [Indonesian](README.id.md) · [언어](README.ko.md) · [日本語](README.ja.md) · [简体中文](README.zh_cn.md) · [繁体中文](README.zh_tw.md) · [Norwegian](README.nb.md) · [Dutch](README.nl.md) · [Polish](README.pl.md) · [Portuguese](README.pt_PT.md) · [Swedish](README.sv.md) · [ภาษาไทย](README.th.md) · [Turkish](README.tr.md) · [Ukrainian](README.uk.md) · [Vietnamese](README.vi.md)

| ![Whisper Mate icon](../images/appicon-128x128.png) | Whisper Mate verandert audio en video in verzorgde ondertitels, vertalingen en exports – allemaal lokaal verwerkt op je Mac zodat elk project privé blijft. Download via de macOS App Store: [Whisper Mate](https://apps.apple.com/us/app/id6450404233) |
| --- | --- |

## 🔑 Belangrijkste voordelen
- **Privacy by design**: Transcriberen, vertalen en exporteren gebeurt volledig op je eigen apparaat; gevoelige media verlaat je Mac nooit.
- **Kies de beste engine**: Schakel tussen FluidAudio, whisper.cpp, WhisperKit, Apple’s on-device AI (macOS 26+) en Kokoro English Speech Synthesis voor voice-overwerk.
- **Live productieproces**: Houd opnames, ondertitels, vertalingen, zwevende previews en automatiseringsketens in realtime in de gaten.
- **Preview zoals het eindresultaat**: Controleer typografie, lettergrootte en regelafstand van ondertitels direct in de ingebouwde speler.
- **Uitbreidbare tooling**: Combineer templates, JavaScript-processors, woordenlijsten en regex-transformaties om complexe opschoning te automatiseren.
- **Speciaal ondertiteldesk**: Beheer bewerken, reviewen, batch-export/import en Final Cut Pro-, QuickLook-, GIF- en MKV-pijplijnen op één plek.

## 🔥 Kernfunctionaliteit
- **Batch- en live-transcriptie**: Verwerk grote assets, live-microfoonopnamen, audio-routing van apps en simultane vertaling.
- **Editor voor ondertitels en vertalingen**: Samenvoegen, splitsen, timing aanpassen, Quick-Cut, massaal vervangen en volledige toetsenbordbediening.
- **Meertalige vertaling**: Gebruik DeepL, Apple Translate, GPT, Gemini, DeepSeek, OpenRouter of lokale Ollama-compatibele LLM’s.
- **Automatiseringsplaybooks**: Start scripts, exports, samenvattingen of meldingen automatisch wanneer een taak klaar is.
- **Import-/export-ecosysteem**: Werk naadloos met SRT, VTT, ASS, FCPXML, GIF, PDF, DOCX, MKV, audioclips en meer.
- **Visuele tools**: Zwevende vensters, aanpasbare lettertypes en regelafstanden, tekst-only of gesplitste weergave met één klik.
- **LLM-samenwerking**: Verbind Apple’s on-device modellen, Ollama of externe LLM’s voor polishing, samenvattingen of scriptfixes.
- **Projectcontrole & snapshots**: Groepeer projecten, prioriteer wachtrijen, maak omkeerbare snapshots, archiveer veilig en monitor externe mappen.
- **Video-postproductie**: Quick Cut waveform editing, hoofdstukken, scenekeyframes, GIF/MP4-export en hard-sub-encoding.

## 🚀 Toepassingen
- Ondertiteling, vertaling en voice-over voor film, podcast en interviews.
- Real-time notities en samenvattingen voor vergaderingen, lessen of livestreams.
- Mediateams die meertalige batches verwerken met end-to-end automatisering.
- Academische of onderzoeksteams die spraakcorpora archiveren en annoteren met snapshots.
- Marketing- en social-teams die long-form video omzetten naar shorts, GIF’s of ingesproken clips.
- Klantenservice-, sales- of juridische opnames die voldoen aan complianceregelgeving met hoogwaardige audittrails.

## 📋 Documentatie
- [FAQ & tips](../docs/faq-and-tips.md)
- [Aangepaste exporttemplates](../docs/custom-export-templates.md)
- [LLM-integraties (samenvattingen, QA, fixes)](../docs/llm-integrations.md)
- [Video’s met ondertitels exporteren](../docs/exporting-subtitled-videos.md)
- [Mapbewaking & automatische transcriptie](../docs/directory-monitoring.md)
- [Realtime transcriptie](../docs/real-time-transcription.md)
- [Segmenten hertranscriberen](../docs/segment-retranscription.md)
- [Presetbeheer](../docs/preset-management.md)
- [Woordenlijst-workflows](../docs/dictionary-workflows.md)
- [Snapshot best practices](../docs/snapshot-best-practices.md)
- [Multiprojectbeheer & zoeken](../docs/multi-project-management.md)
- [Volledig geautomatiseerde workflowvoorbeelden](../docs/fully-automated-workflows.md)

## 💬 Support & feedback
- Stuur feedback via het **About**-menu in de app of open een issue op GitHub zodat we de voortgang samen kunnen volgen.

## 🏞️ App-schermen
![Projects](../images/v9-whispermate_main.webp)
![Subtitle Editor](../images/v9-sub-editor.webp)
![Live Transcription](../images/v9-live-video-transcription.webp)
![Embed Video Previewer](../images/v9-floating.webp)
![Re-transcription](../images/v9-retranscription.webp)
![Custom Export Template](../images/v9-custom-template.webp)

## 🔋 Dank aan deze uitstekende open-sourceprojecten

- [![GitHub stars](https://img.shields.io/github/stars/FluidInference/FluidAudio.svg?style=social&label=Star%20FluidAudio)](https://github.com/FluidInference/FluidAudio)
- [![GitHub stars](https://img.shields.io/github/stars/argmaxinc/WhisperKit.svg?style=social&label=Star%20WhisperKit)](https://github.com/argmaxinc/WhisperKit)
- [![GitHub stars](https://img.shields.io/github/stars/ggerganov/whisper.cpp.svg?style=social&label=Star%20whisper.cpp)](https://github.com/ggerganov/whisper.cpp)

<p align="center">
  <a href="https://fluidinference.com"><img src="https://assets.inference.plus/fi-badge.png" alt="Powered by Fluid Inference" height="80"></a>
  <a href="https://github.com/ggerganov/whisper.cpp" style="margin-left: 16px;"><img src="https://user-images.githubusercontent.com/1991296/235238348-05d0f6a4-da44-4900-a1de-d0707e75b763.jpeg" alt="Powered by whisper.cpp" height="80"></a>
  <a href="https://github.com/argmaxinc/WhisperKit" style="margin-left: 16px;"><img src="../images/vender/whisperkit.png" alt="Powered by WhisperKit" height="80"></a>
</p>
